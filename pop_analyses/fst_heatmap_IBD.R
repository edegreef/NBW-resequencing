...
setwd("C:/Users/eveli/Dropbox/NBW-me/reseq_newsnps/fst")
...
sample_info <- read.csv("C:/Users/eveli/Dropbox/NBW-me/sample_info.csv", header=T)
...
save(df, file="NBW_platypus_SNPs.filter1.filter2.ID.autosomes.LDpruned.n39.df.leftjoined.RData")

# Look at pairwise Fst with hierfstat
wc <- pairwise.WCfst(df, diploid = TRUE)
wc
write.csv(wc,"NBW_platypus_SNPs.filter1.filter2.ID.autosomes.LDpruned.n39.FST.csv")
...
IBD <- ggplot(data, aes(x=shelf_distance_km,y=slatkins_fst))+
  geom_smooth(method = "lm", se=TRUE, fill="gray85", formula=y~x, linetype=0)+
  geom_errorbar(aes(ymin=slatkins_ll, ymax=slatkins_ul), size=1, color="black")+
...
# if want to combine heatmap and IBD plot:
library(patchwork)
fst + IBD
ggsave("FST_and_IBD_plot.png", width=7, height=9.5, dpi=300)
...
dist_data <- read.csv("shelf_dist_matrix.csv")
