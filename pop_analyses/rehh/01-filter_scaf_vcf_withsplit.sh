...
vcf=NBW2_SNPS_2M.filter1.miss.biallel.ID.autosomes.SV.37.miss01.mac
...
# Make list of CHROM and POS info for each snp to examine in R
awk '{print $1, $2}' $vcf.min50kb.vcf > SNPS_CHROM_POS_min50kb
sed '/^#/d' SNPS_CHROM_POS_min50kb > SNPS_CHROM_POS_min50kb_list

# Identify contigs with 1 snp from the list of positions
# Going into R for this with simple code:
#library(dplyr)
...
# Made list of single SNPs to filer out (imputation won't work on contigs with 1 snp): single_snp_windows_CHROM_POS
