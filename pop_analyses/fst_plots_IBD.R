...
# load in fst matrix
fst <- read.csv("fst_reich_slat_matrix.csv", header=T)
...
data <- read.csv("Reichs_fst_output_plus.csv", header=T)
data$pair
...
IBD <- ggplot(data, aes(x=shelf_distance_km,y=slatkin_fst))+
  ...
  xlim(900,5700)+
  ylim(0,0.012)+
  geom_text(aes(label=pair), color="black",size=3, hjust=-0.3,vjust=0.5)+
...
latitude_data <- read.csv("lat_diff_matrix.csv")
